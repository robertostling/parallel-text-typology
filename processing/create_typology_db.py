...
def main():
    ...
    uriel_syntax = get_lang2vec_targets(
            text_info,
            l2v.fs_union(['syntax_wals', 'syntax_ethnologue']))
    proj_syntax = read_wordorder_features(text_info, discretize=True)
    proj_syntax.update(read_affix_features(discretize=True))

    proj_cont_syntax = read_wordorder_features(text_info, discretize=False)
    proj_cont_syntax.update(read_affix_features(discretize=False))

    print(len(uriel_syntax), 'URIEL features')
    print(len(proj_syntax), 'projected features')

    for info in text_info.values():
        ancestry = [l.id for l in glottolog_ancestry(info['glottocode'])]
        info['ancestry'] = ancestry

    all_languages = [info['glottocode'] for info in text_info.values()]
    #all_languages = [l.id for l in Glottolog.languoids]
    # We may need to use ISO codes mapped to other levels, so this filter will
    # not work:
    #                 if l.level == 'language']

    independent = independent_languages(all_languages)

    with open(mulres.config.typology_db_path, 'wb') as f:
        pickle.dump(text_info, f)
        pickle.dump(independent, f)
        pickle.dump(uriel_syntax, f)
        pickle.dump(proj_syntax, f)
        pickle.dump(proj_cont_syntax, f)
